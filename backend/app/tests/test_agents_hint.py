...
def test_multi_agent_flow(monkeypatch):
    # Stabilize agents
    monkeypatch.setattr(
        "app.agents.hint_agent.build_hint_agent",
        lambda: type(
            "C", (), {"invoke": lambda self, d: type("H", (), {"hint": "concise hint", "rationale": "mock"})()}
        )(),
    )
    monkeypatch.setattr(
        "app.agents.guess_agent.build_guess_agent",
        lambda: type(
            "C",
            (),
            {"invoke": lambda self, d: type("G", (), {"guess": "entropy", "confidence": 0.9, "rationale": "mock"})()},
        )(),
    )
    monkeypatch.setattr(
        "app.agents.judge_agent.build_judge_agent",
        lambda: type(
            "C", (), {"invoke": lambda self, d: type("J", (), {"correct": True, "score": 1.0, "feedback": "mock"})()}
        )(),
    )
    monkeypatch.setattr(
        "app.evaluation.tool.evaluate_round_tool",
        type(
            "T",
            (),
            {
                "invoke": lambda self, d: type(
                    "R",
                    (),
                    {"accuracy": 1.0, "calibration": 0.9, "creativity": 0.6, "efficiency": 0.7, "feedback": "ok"},
                )()
            },
        )(),
    )
