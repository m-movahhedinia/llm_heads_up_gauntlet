...
def test_heads_up_with_rag_flow(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.engine.nodes.build_faiss_rag_hint_chain",
        lambda texts: type("C", (), {"invoke": lambda self, q: "RAG hint"})(),
    )
    monkeypatch.setattr(
        "app.engine.nodes.build_hint_compressor", lambda rate=0.5: type("C", (), {"invoke": lambda self, t: t})()
    )
    monkeypatch.setattr(
        "app.engine.nodes.build_noise_injector", lambda severity=0.1: type("N", (), {"invoke": lambda self, t: t})()
    )
